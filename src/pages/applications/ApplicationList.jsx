...
import { Home, Eye, Edit, Search, Filter, ChevronLeft, ChevronRight, Trash2 } from 'lucide-react';
import SearchBar from '../../components/Common/SearchBar';
import StatusBadge from '../../components/Common/StatusBadge';
import useDebounce from '../../hooks/useDebounce';
import usePermissions from '../../hooks/usePermissions';
import { useAppDispatch, useAppSelector } from '../../store/hooks';
import { deleteApp, fetchApplications } from '../../store/slices/applicationSlice';
...
const ApplicationList = () => {
  ...
  const canDeleteApplication = user?.role_id === 1;
  ...
  const handleDeleteApplication = async (row) => {
    ...
    const confirmed = window.confirm(
      `Supprimer la candidature de ${getCandidateName(row)} ?`
    );
  ...
  const renderApplicationMobileRow = (row) => (
    <div className="space-y-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-base font-semibold text-gray-900">{getCandidateName(row)}</p>
          <p className="text-sm text-gray-500">{getPostTitle(row)}</p>
        </div>
        <StatusBadge value={row.status} />
      </div>
      <div className="grid grid-cols-1 gap-2 text-sm text-gray-600">
        <div>
          <span className="font-medium text-gray-900">Date :</span> {getApplicationDate(row)}
        </div>
        <div>
          <span className="font-medium text-gray-900">Source :</span> {getSourceLabel(row)}
        </div>
        <div>
          <span className="font-medium text-gray-900">Assigné à :</span> {getAssignedTo(row)}
        </div>
      </div>
        ...
        {loading && validItems.length === 0 ? (
          ...
          <>
            <div className="space-y-4 md:hidden">
              {pagedRows.map((row) => renderApplicationMobileRow(row))}
            </div>
